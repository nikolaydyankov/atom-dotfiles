...
module.exports =
  activate: ->
    ...
    atom.workspaceView.command 'remote-sync:upload-file', (e)->
      findConfig getEventPath(e), ->
        console.log 'callback from findConfig'
        return if checkSetting()
...
findConfig = (filePath, callback) ->
  console.log 'findConfig start'
  configPath = '/Users/dyankov/Documents/Work/salesrobot/dev/.remote-sync.json'

  # Look for .remote-sync.json in filePath's directory
  # If the file doesn't exist, move up the file tree until .remote-sync.json is found
  # If atom.project.getPath() is reached and the file is not found, return false
